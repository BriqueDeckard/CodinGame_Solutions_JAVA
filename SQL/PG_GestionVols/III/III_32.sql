...
SELECT DISTINCT pi.nopilote, COUNT(DISTINCT av.fk_typeappareil) as nbType
FROM pilote pi
INNER JOIN affectation af
ON af.fk_nopilote = pi.nopilote
INNER JOIN avion av
ON af.fk_noavion = av.noavion
GROUP BY pi.nopilote, av.fk_typeappareil
HAVING 
(
...
(
	...
	(
		SELECT (COUNT(DISTINCT ty.code)-1)
		FROM typeappareil ty
	)
);