...
SELECT DISTINCT pil.nopilote, COUNT(DISTINCT avi.fk_typeappareil) as nbType
FROM pilote pil
INNER JOIN affectation aff
ON aff.fk_nopilote = pil.nopilote
INNER JOIN avion avi
ON aff.fk_noavion = avi.noavion
GROUP BY pil.nopilote, avi.fk_typeappareil
HAVING 
(
...
(
	...
	(
		SELECT (COUNT(DISTINCT typ.code)-1)
		FROM typeappareil typ
	)
);
