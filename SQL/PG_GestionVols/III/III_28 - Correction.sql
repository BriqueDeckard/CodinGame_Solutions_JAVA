...
FROM
	avion
	INNER JOIN affectation aff ON aff.fk_noavion = avion.noavion
	INNER JOIN vol ON fk_novol = novol
	INNER JOIN pilote ON aff.fk_nopilote = pilote.nopilote
	INNER JOIN ville vildep ON fk_vildep = vildep.code
	INNER JOIN ville vilarr ON fk_vilarr = vilarr.code
WHERE
	piloote.fk_ville = vildep.code;